...
class PostgreSQL:
    ...
    def create_table(self, table_name):
        try:
            ...
            self.logger.error(f"Error creating table {table_name} in PostgreSQL: {str(e)}")
    ...
    def insert_data(self, table_name, records):
        try:
            for record in records:
                sf_id = record['Id']
                self.cursor.execute(f"INSERT INTO {table_name} (sf_id) VALUES (%s);", (sf_id,))
            self.connection.commit()
            self.logger.info(f"inserted data in {table_name}")
        except Exception as e:
            self.logger.error(f"Error saving records to PostgreSQL table {table_name}: {str(e)}")
    ...
    def get_ids_from_db(self, table_name):
        try:
            ...
            self.logger.error(f"Error fetching IDs from table {table_name} in PostgreSQL: {str(e)}")
            return []
